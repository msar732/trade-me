...
class AdvancedRecommendationEngine:
    ...
    def get_popular_items(self, limit):
        ...
        from listings.models import BaseListing
        popular = BaseListing.objects.filter(status='active').order_by('-view_count')[:limit]
        return [item.id for item in popular]
...
def retrain_ai_models():
    ...
    try:
        ...
        return {
            'success': True,
            'message': 'All AI models retrained successfully'
        }
...
def calculate_overall_ai_score(fraud_analysis, price_analysis):
    """
    Calculate overall AI score for a listing
    """
    # Fraud score (lower is better)
    fraud_score = 1.0 - fraud_analysis['overall_score']
    
    # Price confidence (higher is better)
    price_confidence = price_analysis.get('confidence', 0.5)
    
    # Combine scores
    overall_score = (fraud_score * 0.7) + (price_confidence * 0.3)
    
    return min(max(overall_score, 0.0), 1.0)

# Utility functions
def get_ai_insights(listing):
    """
    Get comprehensive AI insights for a listing
    """
    fraud_detector = AdvancedFraudDetectionSystem()
    price_predictor = AdvancedPricePredictionEngine()
    ...
    return {
        'fraud_analysis': fraud_analysis,
        'price_analysis': price_analysis,
        'overall_ai_score': calculate_overall_ai_score(fraud_analysis, price_analysis),
        'recommendations': get_ai_recommendations(listing)
    }

def get_ai_recommendations(listing):
    """
    Get AI-powered recommendations for improving a listing
    """
    recommendations = []
    
    # Price recommendations
    if hasattr(listing, 'price') and listing.price:
        price_analysis = AdvancedPricePredictionEngine().predict_price(listing)
        predicted_price = price_analysis['predicted_price']
        current_price = float(listing.price)
        
        if current_price > predicted_price * 1.2:
            recommendations.append({
                'type': 'price',
                'message': f'Consider reducing price from ₹{current_price:,.0f} to around ₹{predicted_price:,.0f} for better visibility',
                'priority': 'high'
            })
        elif current_price < predicted_price * 0.8:
            recommendations.append({
                'type': 'price',
                'message': f'Your price of ₹{current_price:,.0f} seems low. Market suggests ₹{predicted_price:,.0f}',
                'priority': 'medium'
            })
    
    # Image recommendations
    if hasattr(listing, 'images'):
        image_count = listing.images.count() if hasattr(listing.images, 'count') else 0
        if image_count == 0:
            recommendations.append({
                'type': 'images',
                'message': 'Add high-quality images to increase trust and visibility',
                'priority': 'high'
            })
        elif image_count < 3:
            recommendations.append({
                'type': 'images',
                'message': 'Add more images to showcase your item better',
                'priority': 'medium'
            })
    
    # Description recommendations
    if hasattr(listing, 'description'):
        desc_length = len(listing.description) if listing.description else 0
        if desc_length < 100:
            recommendations.append({
                'type': 'description',
                'message': 'Write a more detailed description to attract buyers',
                'priority': 'high'
            })
        elif desc_length > 2000:
            recommendations.append({
                'type': 'description',
                'message': 'Consider shortening your description for better readability',
                'priority': 'low'
            })
...
if __name__ == "__main__":
    print("=" * 80)
    print("🤖 ADVANCED AI ENGINE - INDIA TRADE MARKETPLACE")
    print("=" * 80)
    print("✅ AI Models Loaded:")
    print("- Advanced Recommendation Engine")
    print("- Multi-layered Fraud Detection System")
    print("- AI-powered Price Prediction Engine")
    print("- Real-time Analysis & Insights")
    print("=" * 80)
    print("🚀 Features:")
